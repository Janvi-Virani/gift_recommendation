...
<!-- birthdaysection  -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','friend') or die("connection faild!");
...
<!-- bparty-->
        ...
        <div class="gallery-section">
            <div class="gallery-wrap">
                <div id="bparty">
                <div class="two alt-two">
                <h1>BACHELER PARTY</h1>
                </div>
                <div class="gallery-images">
        <?php  
          $conn=mysqli_connect("localhost",'root','','friend') or die("connection faild!");
         $res=mysqli_query($conn,"SELECT * FROM `bparty` ORDER BY `bparty`.`like` DESC") or die("Query error!");
         if(isset($_POST['submit_like']))
         {
             $id=$_POST['id'];
             $sq="SELECT `like` FROM `bparty` WHERE  id={$id}";            
             $res=mysqli_query($conn,$sq) or die('querry error!!');
             $pre_like=mysqli_fetch_assoc($res);
             $pre_like=$pre_like['like']+1;    
           $sql="UPDATE `bparty` SET `like`='{$pre_like}' WHERE id=$id";
           if(mysqli_query($conn,$sql) or die("Query erroro"))
           {
               header("Location: ./friend.php");
           }  
