        ...
        <div class="wrapper">
            ...
            <div class="shadow">
                <ul>
                    ...
                    <li><a href="#school">First Day Of School</a></li>
...
<!-- birthdaysection  -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','kids') or die("connection faild!");
...
<!-- first day of school -->
        ...
        <?php  
         ...
         $res=mysqli_query($conn,"SELECT * FROM `school` ORDER BY `school`.`like` DESC") or die("Query error!");
         if(isset($_POST['submit_like']))
         {
             $id=$_POST['id'];
             $sq="SELECT `like` FROM `school` WHERE  id={$id}";            
             $res=mysqli_query($conn,$sq) or die('querry error!!');
             $pre_like=mysqli_fetch_assoc($res);
             $pre_like=$pre_like['like']+1;     
           $sql="UPDATE `school` SET `like`='{$pre_like}' WHERE id=$id";
           if(mysqli_query($conn,$sql) or die("Query erroro"))
           {
               header("Location: ./kids.php");
           }  
