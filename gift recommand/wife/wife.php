...
<!-- birthdaysection  -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','wife') or die("connection faild!");
...
<!-- sorrow -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','wife') or die("connection faild!");
         $res=mysqli_query($conn,"SELECT * FROM `sorrow` ORDER BY `sorrow`.`like` DESC") or die("Query error!");
         if(isset($_POST['submit_like']))
         {
             $id=$_POST['id'];
             $sq="SELECT `like` FROM `sorrow` WHERE  id={$id}";             
             $res=mysqli_query($conn,$sq) or die('querry error!!');
             $pre_like=mysqli_fetch_assoc($res);
             $pre_like=$pre_like['like']+1;     
           $sql="UPDATE `sorrow` SET `like`='{$pre_like}' WHERE id=$id";
           if(mysqli_query($conn,$sql) or die("Query erroro"))
           {
               header("Location: ./wife.php");
           }  
...
<!-- anniversary -->
 ...
 <!-- engage -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','wife') or die("connection faild!");
         $res=mysqli_query($conn,"SELECT * FROM `engage` ORDER BY `engage`.`like` DESC") or die("Query error!");
         if(isset($_POST['submit_like']))
         {
             $id=$_POST['id'];
             $sq="SELECT `like` FROM `engage` WHERE  id={$id}";            
             $res=mysqli_query($conn,$sq) or die('querry error!!');
             $pre_like=mysqli_fetch_assoc($res);
             $pre_like=$pre_like['like']+1;    
           $sql="UPDATE `engage` SET `like`='{$pre_like}' WHERE id=$id";
           if(mysqli_query($conn,$sql) or die("Query erroro"))
           {
               header("Location: ./wife.php");
           }  
         ...
         while($data=mysqli_fetch_assoc($res)){
            ...
            <div class="gallery">
                        <div class="image-section">
                            ...
                            <form action="<?php echo htmlspecialchars($_SERVER['PHP_SELF']);?>" method="post" >
                            <label for=""><?php echo $data['like']; ?></label>
                                <button type="submit" name="submit_like"><span class="material-icons-round">like</span></button>
                                <input type="hidden" name="id" value="<?php echo $data['id'];  ?>">
                            </form>
                        </div>
                        </div>
...
<!-- valentine -->
        ...
        <?php  
          $conn=mysqli_connect("localhost",'root','','wife') or die("connection faild!");
         $res=mysqli_query($conn,"SELECT * FROM `valentine` ORDER BY `valentine`.`like` DESC") or die("Query error!");
         if(isset($_POST['submit_like']))
         {
             $id=$_POST['id'];
             $sq="SELECT `like` FROM `valentine` WHERE  id={$id}";            
             $res=mysqli_query($conn,$sq) or die('querry error!!');
             $pre_like=mysqli_fetch_assoc($res);
             $pre_like=$pre_like['like']+1;     
           $sql="UPDATE `valentine` SET `like`='{$pre_like}' WHERE id=$id";
           if(mysqli_query($conn,$sql) or die("Query erroro"))
           {
               header("Location: ./wife.php");
           }  
